...
const VIETNAM_BOUNDS: [number, number, number, number] = [101.8, 7.8, 110.4, 23.6];
...
function getProvinceCode(properties?: ProvinceProperties | null) {
    if (!properties?.code) return null;

    return String(properties.code);
}

function getProvinceName(properties?: ProvinceProperties | null) {
    return properties?.ten_tinh ?? properties?.name ?? "";
}
...
export function MapboxCanvas({
    ...
    const initialStyleKeyRef = useRef(styleKey);
    ...
    const onProvinceHoverRef = useRef(onProvinceHover);
    ...
    useEffect(() => {
        ...
        onProvinceHoverRef.current = onProvinceHover;
    ...
    const setupChoroplethLayers = useCallback((map: mapboxgl.Map) => {
        ...
        map.on("mousemove", LAYER_IDS.fill, (e) => {
            ...
            const code = getProvinceCode(e.features?.[0]?.properties as ProvinceProperties | undefined);
            setHoveredCode(code);
            onProvinceHoverRef.current?.(code);
        });

        map.on("mouseleave", LAYER_IDS.fill, () => {
            map.getCanvas().style.cursor = "";
            setHoveredCode(null);
            onProvinceHoverRef.current?.(null);
        });
        ...
        map.on("click", LAYER_IDS.fill, (e) => {
            const code = getProvinceCode(e.features?.[0]?.properties as ProvinceProperties | undefined);
            if (code) {
                onProvinceClickRef.current?.(code);
            }
        });
    }, []);
    ...
    useEffect(() => {
        ...
        const map = new mapboxgl.Map({
            ...
            style: MAP_STYLES[initialStyleKeyRef.current],
        ...
        map.fitBounds(VIETNAM_BOUNDS, { padding: 28, animate: false });
    ...
    }, [mapError, mapboxToken, setupChoroplethLayers]);
    ...
    useEffect(() => {
        ...
        if (!selectedProvinceCode) {
            ...
            map.fitBounds(VIETNAM_BOUNDS, { padding: 28, duration: 800 });
            return;
        }
    ...
    useEffect(() => {
        const map = mapRef.current;
        if (!map || !map.getLayer(LAYER_IDS.fill) || !map.getLayer(LAYER_IDS.borders)) return;
        ...
        if (filteredProvinceCodes && filteredProvinceCodes.length > 0) {
            opacityCases.push(
                ["match", ["get", "code"], filteredProvinceCodes, false, true],
                0.05
            );
        }
    ...
    useEffect(() => {
        ...
        const onMove = (e: mapboxgl.MapMouseEvent) => {
            ...
            const feature = geojsonRef.current?.features.find(
                (item) => getProvinceCode(item.properties as ProvinceProperties | undefined) === hoveredCode
            );
            const name = getProvinceName(feature?.properties as ProvinceProperties | undefined);
            if (!name) {
