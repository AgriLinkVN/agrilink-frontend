...
import {
  Users, Package, AlertTriangle, ShieldCheck, Lock, Check, X,
} from "lucide-react";
import {
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-8">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <BarChart
                  data={
                    stats
                      ? [
                          { name: "Tổng", value: stats.totalProducts, fill: "#16a34a" },
                          { name: "Chờ duyệt", value: stats.pendingProducts, fill: "#f59e0b" },
                          { name: "Tranh chấp", value: stats.openDisputes, fill: "#ef4444" },
                        ]
                      : []
                  }
                >
            ...
            {/* Pending profiles by role */}
            <div className="bg-white rounded-xl border border-hairline card-shadow p-5">
              <h2 className="font-semibold text-ink mb-4">
                Hồ sơ chờ duyệt ({pendingTotal})
              </h2>
              {stats && pendingTotal > 0 ? (
                <ResponsiveContainer width="100%" height={240}>
                  <PieChart>
                    <Pie
                      data={[
                        { name: "Nông dân", value: stats.pendingProfiles.farmer },
                        { name: "HTX", value: stats.pendingProfiles.cooperative },
                        { name: "Doanh nghiệp", value: stats.pendingProfiles.enterprise },
                        { name: "NCC", value: stats.pendingProfiles.supplier },
                      ].filter((d) => d.value > 0)}
                      dataKey="value"
                      nameKey="name"
                      cx="50%"
                    ...
                    >
                      {["farmer", "cooperative", "enterprise", "supplier"].map((role) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            {[
              ...
              { label: "Duyệt sản phẩm", href: "/dashboard/admin/products", icon: Package },
              { label: "Xử lý tranh chấp", href: "/dashboard/admin/disputes", icon: AlertTriangle },
              { label: "Cấu hình hệ thống", href: "/dashboard/admin/config", icon: ShieldCheck },
            ].map(({ label, href, icon: Icon }) => (
              <Link
                key={href}
                href={href}
                className="bg-white rounded-xl border border-hairline card-shadow p-4 flex items-center gap-3 hover:border-primary transition-colors"
              >
                <div className="w-10 h-10 rounded-lg bg-surface-green flex items-center justify-center">
