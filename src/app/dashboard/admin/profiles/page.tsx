...
export default function AdminProfilesPage() {
  ...
  const fetchData = useCallback(() => {
    ...
    api.get<any>("/admin/pending-profiles", token)
      .then((d) => setData(d))
      .catch(() => {})
  ...
  const handleAction = async (type: string, profileId: string, isApproved: boolean) => {
    ...
    try {
      await api.patch(`/admin/profiles/${type}/${profileId}/verify`, {
        isApproved,
            ...
            return (
              <div key={`${p._type}-${p.id}`} className="bg-white rounded-xl border border-hairline card-shadow p-5">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleAction(p._type, p.id, true)} className="flex-1 h-9 rounded-lg bg-surface-green text-primary text-sm font-semibold flex items-center justify-center gap-1 hover:bg-primary hover:text-white transition-colors">
                    <Check size={14} /> Duyệt
                  </button>
                  <button onClick={() => handleAction(p._type, p.id, false)} className="flex-1 h-9 rounded-lg bg-[#FEE2E2] text-error text-sm font-semibold flex items-center justify-center gap-1 hover:bg-error hover:text-white transition-colors">
                    <X size={14} /> Từ chối
                  </button>
                </div>
              </div>
